...
class OrquestradorMaster:
    def __init__(self):
        # BUSCA DE CHAVES À PROVA DE BALAS (Evita o erro de chave vazia)
        try:
        ...
        for tentativa in range(tentativas):
            try:
                chat_completion = self.groq_client.chat.completions.create(
                    ...
                    temperature=0.85 # Temperatura alta para garantir máxima humanização
                )
    ...
    def executar_consenso_hidra(self, questao_usuario: str, dados_da_internet: str) -> str:
        ...
        # Roteamento Inteligente: Tenta a Groq (Mais humana e rápida), depois o Gemini
        if self.groq_key:
            try:
            ...
            erro_groq = "Chave GROQ_API_KEY ausente ou incorreta no cofre (Secrets)."
        ...
        if self.gemini_key:
            ...
            erro_gemini = "Chave GEMINI_API_KEY ausente ou incorreta no cofre (Secrets)."

        # Se tudo falhar, a Autocura revela o diagnóstico real para você consertar
        return (
            f"⚠️ **DIAGNÓSTICO DO MOTOR NEURAL** ⚠️\n\n"
            f"O Sabixão não encontrou as chaves no cofre do Streamlit ou as IAs recusaram a conexão.\n"
            f"🔴 **Status Groq:** `{erro_groq}`\n"
            f"🔵 **Status Gemini:** `{erro_gemini}`\n\n"
            f"**Ação Necessária:** Vá no painel do Streamlit Cloud > Settings > Secrets e verifique se as chaves estão coladas EXATAMENTE assim:\n"
